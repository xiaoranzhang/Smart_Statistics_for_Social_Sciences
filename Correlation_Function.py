...
def standard_deviation():
    ...
    out_brackets = 1 / (n - 1) * inner_brackets    # 计算方差： [(x1-x)^2+(x2-x)^2+......(xn-x)^2]/(n) （x为平均数）
    stard_devia = out_brackets ** o.5    # 计算标准差（方差的开方） 或者调math模块：math.sqrt()
...
standard_deviation()    # 调用第一个函数 average()
